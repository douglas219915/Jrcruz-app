...
texts = {
    "Español": {
        ...
        "btn_save": "Guardar en Google Drive", "btn_edit": "Actualizar en Drive", "btn_pdf": "Descargar Recibo PDF",
    ...
    "English": {
        ...
        "btn_save": "Save to Google Drive", "btn_edit": "Update to Drive", "btn_pdf": "Download Receipt PDF",
...
if "📝" in choice:
    ...
    if st.button(t["btn_save"]):
        ...
        try:
            ...
            st.success(t["exito"])
        except: st.error("Asegúrate de que la pestaña 'Estimados' exista en Drive.")

# --- MODULO 2: HISTORIAL Y PAGOS ---
elif "📋" in choice:
    st.title(t["menu"][1])
    try:
        # Cargamos directamente desde Google Sheets
        df_h = conn.read(worksheet="Estimados").astype(str)
...
# --- MODULOS: CITAS Y NÓMINA (PARA DRIVE) ---
elif "📅" in choice:
    st.title(t["m_citas"])
    with st.form("c"):
