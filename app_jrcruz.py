...
# --- MODULO 2: HISTORIAL Y PAGOS (SOLUCIÓN AL TYPEERROR) ---
elif "📋" in choice:
    st.title(t["menu"][1])
    file = "historial_final.csv"
    if os.path.exists(file):
        # Cargamos todo como STRING primero para evitar que Pandas bloquee columnas
        ...
        if sel_c != "":
            ...
            # Convertimos valores a números solo para el cálculo
            val_total = float(df_h.loc[idx, "Total"])
            deps_list = [float(d) for d in str(df_h.loc[idx, "Depositos"]).split(";") if d]
            ...
            if st.button(t["btn_edit"]):
                # SOLUCIÓN CRÍTICA: Convertimos la columna a objeto antes de asignar el texto
...
# --- MODULOS: CITAS Y NÓMINA (TRADUCCIÓN REVISADA) ---
elif "📅" in choice:
    st.title(t["m_citas"])
    with st.form("c"):
